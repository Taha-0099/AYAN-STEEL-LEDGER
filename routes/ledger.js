...
//  - ?ledgerType=SALES|PURCHASE   (legacy alias)
//  - ?from=YYYY-MM-DD&to=YYYY-MM-DD
// =====================================================
router.get("/", async (req, res) => {
  try {
    const { accountName, from, to, category, ledgerType } = req.query;
    ...
    // ✅ category preferred, ledgerType supported as alias
    const normalized =
      normalizeCategory(category) || normalizeCategory(ledgerType);
    if (normalized) {
...
router.post("/", async (req, res) => {
  try {
    ...
    const normalized = normalizeCategory(category) || normalizeCategory(ledgerType);

    const entry = new LedgerEntry({
      category: normalized || "SALE",
...
// PUT /api/ledger/:id
// ✅ SAFE PARTIAL UPDATE
// Supports category + ledgerType alias
// =====================================================
router.put("/:id", async (req, res) => {
  try {
    ...
    const normalized = normalizeCategory(category) || normalizeCategory(ledgerType);
    if (normalized) update.category = normalized;
